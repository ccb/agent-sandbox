...
def test_layer_stack_order():
    ...
    assert names[ef + 1:ef + 5] == [
        "fisher_floor", "fisher_walls", "fisher_rugs", "fisher_furniture"]
...
def test_furniture_and_rugs_only_on_interior():
    tmj = _fresh()
    ff.apply(tmj, MATRIX); ff.apply_walls(tmj, MATRIX); ff.apply_furniture(tmj, MATRIX)
...
def test_rug_runners_fill_their_boxes():
    ...
    for name, (c0, r0, c1, r1) in boxes:
        in_box = [(c, r) for r in range(r0, r1 + 1) for c in range(c0, c1 + 1)
                  if (c, r) in rugs]
...
def test_idempotent():
    a = _fresh()
    ff.apply(a, MATRIX); ff.apply_walls(a, MATRIX); ff.apply_furniture(a, MATRIX)
    b = _fresh()
    ff.apply(b, MATRIX); ff.apply_walls(b, MATRIX); ff.apply_furniture(b, MATRIX)
