...
ROOM_IDS = list(range(11500, 11505))
ROOMS = {"Stage", "Auditorium", "West Foyer", "Lobby", "East Foyer"}
...
def test_5_irvine_room_arenas_present(tmp_path):
    mdir, _ = _run(str(tmp_path))
    rows = _arena_blocks(mdir)
    ir = [r for r in rows if r[2] == IRVINE and r[3] not in ("grounds", "lobby")]
    assert len(ir) == 5
    assert sorted(int(r[0]) for r in ir) == ROOM_IDS
    assert {r[3] for r in ir} == ROOMS
    assert any(r[2] == IRVINE and r[3] == "lobby" for r in rows)


def test_subboxes_grouped_no_numbered_arenas(tmp_path):
    # Stage 1/2/3, West/East Foyer 1/2 must merge into one arena each -- no arena
    # name should end in a digit
    mdir, _ = _run(str(tmp_path))
    rows = _arena_blocks(mdir)
    bad = [r[3] for r in rows if r[2] == IRVINE and re.search(r"\d$", r[3])]
    assert not bad, f"ungrouped numbered arenas: {bad}"
...
def test_partition_walls_block(tmp_path):
    ...
    interior = fi.irvine_interior_cells(tmj, mdir)
    walls = fi.irvine_wall_cells(tmj, interior, W, H)
    assert walls, "expected Irvine partition walls"
    coll = _read_flat(os.path.join(mdir, "maze", "collision_maze.csv"))
    for x, y in walls:
        assert coll[y * W + x] == "1", f"partition cell {(x, y)} is not blocking"


def test_every_irvine_room_reachable_from_a_door(tmp_path):
    mdir, _ = _run(str(tmp_path))
    coll = _read_flat(os.path.join(mdir, "maze", "collision_maze.csv"))
