...
func _on_room_changed(room_id: String, _scene_path: String) -> void:
	...
	if player != null:
		_fan_room(room_id, line, [], player.position)   # the event happens where the entering player stands
	else:
...
func _on_event(event: Dictionary) -> void:
	...
	if t == "item_gathered" or t == "material_deposited":
		...
		_fan_room(actor.room, act_line, [actor.id], actor.position)
		return
	...
	if t == "ability_cast_started":
		...
		_fan_room(caster.room, cast_line, [caster.id], caster.position)
		return
	...
	if t == "deed_performed":
		...
		if doer == null or deed_line == "":
			return
		_fan_room(doer.room, deed_line, [doer.id], doer.position)
		return
	...
	if t == "agent_attacked":
		...
		if target.id != Agents.PLAYER_ID:
			...
			target.remember(line)
		_fan_room(target.room, line, [target.id], target.position)   # witnessed where the victim stands
	else:  # agent_downed — a person struck down is seen by whoever present can see the body fall
		_fan_room(target.room, "%s was struck down." % _name(target.id), [target.id], target.position)
...
func _fan_room(room: String, line: String, except: Array, event_pos: Variant = null) -> void:
	for a in Agents.all():
