...
def test_event_state_contract_accepts_a_null_actor():
    """World-level stimuli -- ambient sound, POST /world/event, the boil arc's
    `boiled` event -- legitimately carry actor=None, and RunStore deliberately
    persists them. The pinned contract must agree, or exporting such a run
    crashes #584's audit and a TS consumer typed `actor: string` receives null
    (#631)."""
    from backend.contract_models import EventState
    ...
    ev = {
        ...
        "payload": {"item": "pot of water"},
    }
    model = EventState.model_validate(ev)
    assert model.actor is None
    # A named actor still validates unchanged.
    assert (
        EventState.model_validate(dict(ev, actor="Diego Torres")).actor
        == "Diego Torres"
    )


def test_boil_scenario_bake_validates_against_the_contract(tmp_path, monkeypatch):
    """#631 end-to-end: the boil arc (the #595/#624 fixture) pins a `boiled`
    event with actor=None. Its exported replay must validate against the pinned
    Replay contract -- before the fix `Replay.model_validate` raised on the null
    actor, crashing #584's audit of exactly this fixture. Extends the #307
    conformance coverage past the default penn scenario, as #631 asks."""
    from backend.contract_models import EventState, Replay
    from backend.penn import export_replay, generate_penn_replay
    from backend.run_store import RunStore

    out = tmp_path / "boil.json"
    runs = tmp_path / "runs"
    monkeypatch.setattr(
        sys,
        "argv",
    ...
    assert generate_penn_replay.main() == 0
    replay = json.loads(out.read_text())
    # The arc's world-level `boiled` event is present and actor-less.
    boiled = [e for e in replay["events"] if e["action"] == "boiled"]
    assert boiled and all(e["actor"] is None for e in boiled)
    # Every event row conforms -- null actor and all.
    for e in replay["events"]:
        EventState.model_validate(e)
    # ...and the whole exported replay validates (the #584 audit path).
    store = RunStore(runs)
    (run,) = store.list_runs()
    exported = export_replay.build_replay(store, run["id"])
    assert exported == replay
