...
(`backend/world_map.py`), which loads the Penn world matrix — a folder of CSVs
at `backend/penn/the_upenn/matrix/` that encode collision tiles, sectors
(buildings), arenas (interior/exterior), game objects, spawning locations, and
furniture. The matrix is generated by the tooling under `tools/geo/` (see
`tools/geo/README.md` for the regen pipeline).

### Penn matrix files in `special_blocks/`

| File | Contents |
|------|----------|
| `world_blocks.csv` | world ID (e.g. "UPenn") |
| `sector_blocks.csv` | building IDs → labels (e.g. "Van Pelt Library") |
| `arena_blocks.csv` | interior/exterior arena IDs → labels (e.g. "lobby", "grounds") |
| `game_object_blocks.csv` | named game objects (furniture, doors, signs) |
| `spawning_location_blocks.csv` | agent spawn points |
| `furniture_blocks.csv` | walkable furniture tile positions (#537) — consumed by the path renderer to draw furniture under moving sprites |
| `furniture_spots.csv` | one interaction spot per furniture piece (`world, sector, arena, x, y`) — walk-on pieces land on the piece itself, others on the adjacent floor. Consumed by `WorldMap.furniture_spots` |

The path builder routing agents to a building prefers a furniture spot (standing
at the desk) when available, else falls back to the arena centroid.
