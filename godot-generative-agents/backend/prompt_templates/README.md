...
| `believability_rubric.prompty` | `eval/believability.py` — `LlmJudge.score_agent()` | **Real model prompt.** The offline believability audit's judge rubric (#584): one agent's evidence digest (assembled in code by `evidence_text()`) plus the six scoring dimensions (including `world_grounding`, #780, and `memory_carry`, #814), graded via the `grade_believability` tool. Never rendered during a live sim — the audit reads a *finished* run's exported artifacts; on a declined/malformed reply (or under the free mock provider) the deterministic heuristic scores instead. |
