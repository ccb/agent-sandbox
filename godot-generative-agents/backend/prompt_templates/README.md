...
| `decide_context.prompty` | `cognition.py` — `decide_context_block()`, folded into `observe_and_decide`'s observation when the step loop threads a `SimClock` (#580) | **Real model prompt context.** The always-on decide slice: `Right now it is <time>.` + the plan's current stop (with planned minutes) + `This has been your current stop for <N> min.` Deliberately not "you have been here": an agent that wanders off-plan never arrives, nothing re-anchors the clock, and the stop it is neglecting keeps counting (#826) — so the second phrasing would be false for exactly the agent the sentence exists to warn. Appended after the environment text, so the deterministic mock never reads it. While a completed stop's pointer is held by the next stop's `start_hour` (#838), the elapsed clause is *replaced* by `You have already finished this stop.` plus `Your next stop is <activity> at <place>, starting at <hour> (<N> min from now).` — or `, due now.` once that hour has passed (#826). The elapsed minutes are true in that state but read as debt: batch 5 had an agent walk 62 sim-minutes back to an errand it had already finished, quoting that clause as its reason. Inside the run's final two hours the block also says when the day ends and in how many minutes (#891) — batch 8 had an agent start a ~50-minute walk 23 minutes before a day end nothing had told it about; needs the loop to thread `num_steps`, so the bake never renders it. Past 90 unchanged minutes on one performed activity the block adds `You have been doing exactly this -- "<activity>" -- for <N> min` (#905), plus `, and <Name> is not here` when the activity names an absent cast member — batch 10 had an agent re-choose a person-contingent wait for 340 minutes because the decayed premise never appeared in its context; needs the loop to thread `act_since`, so the bake never renders this either. Mid-walk toward the current scheduled stop the elapsed clause is replaced by `You are walking there now, about <M> min from arrival.` (#916) — mid-walk, the elapsed number reads as time already spent at the stop, and batch 11's Tanaka turned around 11 tiles from the door of a correct 68-minute leg because of it. |
...
| `walk_minutes.prompty` | `cognition.py` — `walk_minutes_line()`, folded into `observe_and_decide`'s observation when the step loop threads a `SimClock` and the world has a map (#826) | **Real model prompt context.** What a walk costs from where the agent stands: `Walking from here takes at least about: <name> <N> min; …`, nearest first, including the place it is already in at 0 min. Penn legs run ~57 sim-minutes but the exits list prices them all alike, so "quick" errands were free. A per-destination BFS distance field over the collision grid (`WorldMap.walk_steps_from`, #866) — walks around walls, replacing the Chebyshev bbox bound that under-reported ~2× and changed a batch-6 decision; "at least about" stays because the patched `walk_path` routes a few tiles past the nearest destination tile. Appended after the environment text; the deterministic mock never reads it. |
| `abandoned_walks.prompty` | `cognition.py` — `abandoned_walks_block()`, folded into `observe_and_decide`'s observation when the step loop threads a `SimClock` and its travel branch has recorded an abandoned cross-building leg (#933) | **Real model prompt context.** The dither damper: `You recently abandoned a walk to <building> (<N> min ago) without doing anything there, and nothing new has come up about that place since. Do not travel back there without a new reason you can state — walking back and forth wastes the day.` (pluralized when several legs qualify). #916's mechanical fix held, yet #878 candidate 1 still dithered Houston ↔ Irvine ↔ College Hall for ~2.5 sim-hours — 12 cross-building retargets, 238 abandoned-leg minutes — because every retarget was a locally plausible decide and nothing in the prompt said the agent had *just given up* on that destination. A leg renders only while fresh (`ABANDONED_LEG_WINDOW_MIN`, 45 sim-minutes — sized against the observed ~25-min ping-pong cycle), unjustified (no new non-action memory since the abandonment names the building or any of its rooms — a conversation that does is exactly the "new reason" that lifts it), and off-plan (the current scheduled stop is exempt: the #580 block names it as the plan in the same prompt). A discouragement the model can override with a stated reason, never a hard block. Appended after the environment text; the deterministic mock never reads it. |
