...
| `known_food_location.prompty` | `cognition.py` — `known_food_location_line()`, appended into `observe_and_decide`'s observation on the real-brain tool path (#931 follow-up) | **Real model prompt context.** Campus-wide (not vision-radius-limited) counterpart to `nearby_affordances`: every dining/marketplace location in the game (`Even if it isn't nearby, you know food can be found at: <name> (<tags>); …`), appended only when the character is `is_low_energy` ("hungry"). Same real-brain-only gate as `nearby_affordances`; the deterministic mock never reads it. |
| `known_sleep_location.prompty` | `cognition.py` — `known_sleep_location_line()`, appended into `observe_and_decide`'s observation on the real-brain tool path (#931 follow-up) | **Real model prompt context.** Sleep's counterpart to `known_food_location`: every sleepable location in the game (`Even if it isn't nearby, you know you can sleep at: <name>; …`), appended only when the character is `IS_SLEEPY`. Same real-brain-only gate; the deterministic mock never reads it. |
...
`nearby_affordances`'s, `known_food_location`'s, and `known_sleep_location`'s by
`godot-generative-agents/tests/test_affordance_wiring_613.py`;
