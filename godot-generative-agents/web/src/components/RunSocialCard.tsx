 ...
 * exactly that reason) and the wire can't distinguish resumed.
 */
export function socialView(social: RunSocial | undefined, step: number): SocialView | null {
  if (!social) return null;
  const pairs = (Object.entries(social.by_pair) as [string, number][])
    .sort(([, a], [, b]) => b - a)
    .slice(0, TOP_PAIRS);
  return {
  ...
  return (
    <div className="agent-field social-card">
      ...
      <p className="social-line">
        ...
        conversation{v.conversations === 1 ? "" : "s"}
      </p>
      {v.quiet ? (
        <p className="mem-empty social-quiet">No co-settled moments seen yet this session.</p>
      ) : (
        v.pairs.length > 0 && (
