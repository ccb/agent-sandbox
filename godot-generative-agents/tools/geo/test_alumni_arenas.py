...
def test_alumni_twentytwo_room_arenas(tmp_path):
    mdir = _run(str(tmp_path))
    rows = _arena_blocks(mdir)
    rooms = [
        r
        for r in rows
        if r[2] == "Sweeten Alumni Building" and r[3] not in ("grounds", "lobby")
    ]
    assert len(rooms) == 22
    assert sorted(int(r[0]) for r in rooms) == list(range(12700, 12722))
    names = {r[3] for r in rooms}
    assert "Room 1: Kitchen" in names and "Room 2: BD 1" in names
    # decoys must NOT have become arenas
    assert "Room 1: Wall" not in names and "Brick Wall" not in names


def test_alumni_rooms_reachable_through_a_door(tmp_path):
    mdir = _run(str(tmp_path))
