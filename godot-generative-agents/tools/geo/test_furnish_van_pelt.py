        ...
        for i, v in enumerate(L["data"]):
            if v:
                assert (
                    i % W,
                    i // W,
                ) in interior, f"{n} cell {(i%W,i//W)} outside interior"
...
def test_clips_the_throat_seam_cells():
    ...
    assert (
        1 <= clipped <= 50
    )  # ~20 (10 throat cells x 2 layers); never zero, never large
...
def test_no_phantom_walls_after_apply():
    ...
    fv.apply(tmj, fv.load_asset(HERE), MATRIX)
    coll = (
        open(os.path.join(MATRIX, "maze", "collision_maze.csv"))
        .read()
        .strip()
        .split(", ")
    )
    for name in ("westwing_walls", "eastwing_walls"):
        ...
        for i, v in enumerate(L["data"]):
            if v:
                assert (
                    coll[i] == "1"
                ), f"{name} sprite on walkable cell {(i % W, i // W)}"


def test_idempotent():
    a = _fresh_tmj()
    fv.apply(a, fv.load_asset(HERE), MATRIX)
    b = copy.deepcopy(a)
    fv.apply(b, fv.load_asset(HERE), MATRIX)
