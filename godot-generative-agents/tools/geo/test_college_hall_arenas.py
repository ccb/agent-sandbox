...
ROOM_IDS = list(range(10800, 10811))
ROOMS = {
    "Central Hall",
    "West Wing",
    "East Wing",
...
def test_11_college_room_arenas_present(tmp_path):
    mdir, _ = _run(str(tmp_path))
    rows = _arena_blocks(mdir)
    cr = [r for r in rows if r[2] == COLLEGE and r[3] not in ("grounds", "lobby")]
    assert len(cr) == 11
    assert sorted(int(r[0]) for r in cr) == ROOM_IDS
    assert {r[3] for r in cr} == ROOMS
    assert any(r[2] == COLLEGE and r[3] == "lobby" for r in rows)
...
def test_partition_walls_block(tmp_path):
    ...
    interior = fc.college_interior_cells(tmj, mdir)
    walls = fc.college_wall_cells(tmj, interior, W, H)
    assert walls, "expected College Hall partition walls"
    coll = _read_flat(os.path.join(mdir, "maze", "collision_maze.csv"))
    for x, y in walls:
        assert coll[y * W + x] == "1", f"partition cell {(x, y)} is not blocking"


def test_every_college_room_reachable_from_a_door(tmp_path):
    mdir, _ = _run(str(tmp_path))
    coll = _read_flat(os.path.join(mdir, "maze", "collision_maze.csv"))
    ...
    for rid in ROOM_IDS:
        assert (
            str(rid) in reached
        ), f"College room arena {rid} unreachable through the door"
...
def test_other_buildings_unchanged_regression(tmp_path):
    ...
    def count(b):
        return len([r for r in rows if r[2] == b and r[3] not in ("grounds", "lobby")])

    assert count("Van Pelt Library") == 25
    assert count("Fisher Fine Arts Library") == 6
    assert count("Houston Hall") == 9  # adding College disturbs none of them
