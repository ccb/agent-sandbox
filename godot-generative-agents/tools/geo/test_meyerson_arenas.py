...
ROOM_IDS = list(range(12100, 12106))
ROOMS = {
    "Cafe",
    "Lounge",
    "West Studio",
...
def test_6_meyerson_room_arenas_present(tmp_path):
    mdir, _ = _run(str(tmp_path))
    rows = _arena_blocks(mdir)
    mr = [r for r in rows if r[2] == MEYERSON and r[3] not in ("grounds", "lobby")]
    assert len(mr) == 6
    assert sorted(int(r[0]) for r in mr) == ROOM_IDS
    assert {r[3] for r in mr} == ROOMS
    assert any(r[2] == MEYERSON and r[3] == "lobby" for r in rows)


def test_no_rug_arenas(tmp_path):
    # a 'rug'-named object must never be promoted to an arena
    mdir, _ = _run(str(tmp_path))
    assert not [r for r in _arena_blocks(mdir) if "rug" in r[3].lower()]
...
def test_partition_walls_block(tmp_path):
    ...
    interior = fm.meyerson_interior_cells(tmj, mdir)
    walls = {cell for _side, cell in fm.iter_wall_cells(sections, interior, W)}
    assert walls, "expected Meyerson partition walls"
    coll = _read_flat(os.path.join(mdir, "maze", "collision_maze.csv"))
    for x, y in walls:
        assert coll[y * W + x] == "1", f"partition cell {(x, y)} not blocking"


def test_every_room_reachable_from_a_door(tmp_path):
    mdir, _ = _run(str(tmp_path))
    coll = _read_flat(os.path.join(mdir, "maze", "collision_maze.csv"))
        ...
        for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            ...
            if 0 <= nx < W and 0 <= ny < H:
                ...
                if coll[j] == "0" and not seen[j]:
                    ...
                    q.append((nx, ny))
    for rid in ROOM_IDS:
        assert str(rid) in reached, f"Meyerson room arena {rid} unreachable"
