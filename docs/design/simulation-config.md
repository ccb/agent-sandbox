...
`generative-agents/backend/sim_config.py`. `SimulationConfig` is the
...
```
SimulationConfig                      # generative-agents/backend/sim_config.py
...
| `steps` | int | `360` | Steps to simulate (360 × 10s = 1 hour). `--steps`. | implemented |
...
- Implementation: `generative-agents/backend/sim_config.py`,
  `generative-agents/backend/run_simulation.py`.
