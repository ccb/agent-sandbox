...
**Status:** Core implemented (this PR). Stages 1–5 of §8 shipped:
`text_adventure_games/recording.py` (`RecordingClient`/`ReplayClient`/`seed_world`)
and `text_adventure_games/transcript.py` (`StepRecord`/`TranscriptRecorder`/
`RunRecord`), with `tests/test_recording.py` + `tests/test_transcript.py`. Two
conscious departures from the plan below, both noted inline: cassette matching is
**exact-match keying** (v1), not the keyed-with-normalization of §4A — a
deterministic replay reproduces byte-identical requests, so normalization isn't
needed yet (left as a documented extension point in `request_key`); and the run
record (§4B/§5) is saved as **YAML** (a single human-readable document per run),
while **cassettes stay JSONL** per the §10 invariant. Deferred: wiring `RunRecord`
into #47's runner (no code yet), the golden-transcript regression (§8 stage 6),
and capturing the bare gather-phase decision in `turns.py` (the one transcript gap
in simultaneous mode). **Companion to** `docs/design/planning-benchmark.md`
(issue #47): the benchmark *scores* agent runs; this doc is about making those
runs **re-creatable**.
