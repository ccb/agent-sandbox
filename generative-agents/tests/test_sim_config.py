...
def test_defaults_match_today():
    ...
    assert config.simulation.steps == 360
...
def test_sim_config_retrieval_keeps_replay_byte_identical(world_map):
    ...
    # mock embedding client) reorders the mock-ignored memory block but never the
    # decisions -- the exported frames stay byte-identical to the default run.
