...
agent-sandbox port note
-----------------------
This is a fork of the upstream Generative Agents ``translator/views.py``, copied
here under ``frontend_overrides/`` so it survives ``setup.sh`` (which rsyncs the
upstream tree with --delete, then copies these overrides on top). Every view is
reproduced verbatim EXCEPT ``replay_persona_state``, which is extended to feed the
State Details panel the real memory our sim produces -- see the comment there. If
you bump the upstream clone, re-sync the untouched views from it.
"""
...
def _load_replay_memories(sim_code, step, persona_name):
    """agent-sandbox port: the real memory behind the State Details panel.

    Upstream renders ``a_mem_event/chat/thought`` from the seed
    ``associative_memory/nodes.json``, which our mock sim never writes to -- so
    that section showed up empty. Instead we surface the *same* memory data the
    replay's agent cards show, from the artifacts the exporter already produces:

      * ``retrieved`` -- the records retrieved for THIS step's decision, read from
        the step's movement frame (exactly what the card renders at that step), and
      * ``all`` -- the agent's full stream, from ``personas/<Name>/memory_stream.json``
        (newest first).
    ...
    retrieved = []
    move_file = _first_existing(
        f"storage/{sim_code}/movement/{step}.json",
        f"compressed_storage/{sim_code}/movement/{step}.json",
    )
    try:
        with open(move_file) as json_file:
            frame = json.load(json_file)
        retrieved = frame.get("persona", {}).get(persona_name, {}).get("memories", [])
    except (OSError, ValueError, KeyError):
        retrieved = []

    all_memories = []
    stream_file = _first_existing(
        f"storage/{sim_code}/personas/{persona_name}/memory_stream.json",
        f"compressed_storage/{sim_code}/personas/{persona_name}/memory_stream.json",
    )
    try:
        with open(stream_file) as json_file:
            all_memories = json.load(json_file).get("memories", [])
