...
The **sim engine** is the top-level `gen_agents/` package, run in the repo's `uv`
project env (one level up): `uv run python -m gen_agents.run_simulation`. The
**Django frontend** needs its own Python 3.9 venv (`frontend-venv/`, Django 2.2) —
`run-replay.sh` provisions both. Don't try to run the frontend from the engine env.

## Prompt templates (`gen_agents/prompt_templates/`)
...
`.prompty` files (Jinja + Prompty) under `gen_agents/prompt_templates/`, not as
...
Render with `from gen_agents.prompt_templates import render` →
