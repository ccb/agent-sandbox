...
| `place_grounding.prompty` | `conversation._place_grounding_block` (via `_dialogue_observation`) | the #780 real-vs-off-map place block for a conversing agent. |
| `reflect_system.prompty` | `reflection.py` — `LLMReflector._call()` (both reflection steps) | System message for periodic reflection (issues #84 / #815): synthesize grounded insights while preserving the `[lived]` / `[conversation]` / `[inferred]` / `[intended]` / `[memory]` status added to each numbered record in code. |
